...
class TestDockerfileCompose:
    ...
    def test_docker_compose_file(self):
        ...
        assert "build:" in docker_compose, (
            "Проверьте, что backend/frontend собираются локально, "
            "без обязательного DockerHub-образа"
        )
        assert re.search(r"image:\s+caddy:", docker_compose), (
            "Проверьте, что Caddy добавлен в docker-compose.yaml"
        )
